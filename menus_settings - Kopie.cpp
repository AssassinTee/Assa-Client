...
void CMenus::RenderSettingsGraphics(CUIRect MainView)
{
	...
	/*char bBuf[64];
	str_format(bBuf, sizeof(bBuf), "%i", g_Config.m_UiColor);
	MainView.HSplitTop(19.0f, &Button, &MainView);
	UI()->DoLabelScaled(&Text, bBuf, 13.0f, -1);*/
}
...
{
	CUIRect Button, Label, Keys, Left, Right;
	const float FontSize = 12.0f;
		//MainView.HSplitTop(20.0f, &Button, &MainView);
		MainView.VSplitMid(&Left, &Right);
		Left.VSplitRight(10.0f, &Left, 0);
		Right.VMargin(5.0f, &Right);
		//Button.VSplitLeft(20.0f, 0, &Button);
		/*
		// skin info
		...
		*/
		...
		str_format(bBuf, sizeof(bBuf), "Assabliblablub");//TODO: get score differents delta S
		TextRender()->Text(0, 400*3.0f*Graphics()->ScreenAspect()*1.5f/2 - TextRender()->TextWidth(0, 18.0f + 20.0f * g_Config.m_ClNameplatesSize / 100.0f, bBuf, -1)/2, 400*3.0f*1.5f/2 - (18.0f + 20.0f * g_Config.m_ClNameplatesSize / 100.0f)/2 - 110.0f, 18.0f + 20.0f * g_Config.m_ClNameplatesSize / 100.0f, bBuf, -1);

		RenderTools()->RenderTee(CAnimState::GetIdle(), &OwnSkinInfo, 0, vec2(1, 0), vec2(MainView.x+500.0f, MainView.y+28.0f));
		*/
		Left.HSplitTop(20.0f, &Button, &Left);
		if(DoButton_CheckBox(&g_Config.m_BrSpecialScore, Localize("Show playerscore"), g_Config.m_BrSpecialScore, &Left))
			g_Config.m_BrSpecialScore ^= 1;
		/*
		//Button.VSplitLeft(400.0f, 0, &Button);
		UI()->DoLabel(&Right, Localize("Dynamic camera bind-key:"), 16.0f, -1);
		...
		*/
		//Button.VSplitLeft(-520.0f, 0, &Button);

		Left.HSplitTop(20.0f, &Button, &Left);
		if(g_Config.m_BrSpecialBlood)
		{
			if(g_Config.m_BrSpecialRedBlood)
				TextRender()->TextColor(1.0f, 0, 0, 1.0f);
			else
			{
				int r, g, b;
				r = g_Config.m_BrSpecialColorBlood>>16;
				g = (g_Config.m_BrSpecialColorBlood>>8)&0xff;
				b = g_Config.m_BrSpecialColorBlood&0xff;
				TextRender()->TextColor(r/255.0f, g/255.0f, b/255.0f, 1.0f);
			}
		}
		if(DoButton_CheckBox(&g_Config.m_BrSpecialBlood, Localize("Use Own blood"), g_Config.m_BrSpecialBlood, &Left))
		...
		if(g_Config.m_BrSpecialBlood)
		{
			...
			if(!g_Config.m_BrSpecialRedBlood)
			{
				...
				g = (g_Config.m_BrSpecialColorBlood>>8)&0xff;
				b = g_Config.m_BrSpecialColorBlood&0xff;
				RenderRgbSliders(&Left, &Button, r, g, b, true);//true or false? i don't know
		...
		if(g_Config.m_BrSpecialKillMessage)
		{
			...
			if(DoButton_CheckBox(0 , Localize("Color"), 0, &Button))
				m_SpecialPage = 1;
			MainView.VSplitLeft(-100.0f, &Button, &MainView);*/
		...
		if(g_Config.m_BrSpecialKillAssa)
			TextRender()->TextColor(1,1,0,1);
		if(DoButton_CheckBox(&g_Config.m_BrSpecialKillAssa, Localize("Show score additions"), g_Config.m_BrSpecialKillAssa, &Button))
		...
		if (g_Config.m_BrSpecialWall)
		{
			...
			if (g_Config.m_BrSpecialWallQuads)
			{
				Left.HSplitTop(20.0f, &Button, &Left);
				...
				{
					int r1, g1, b1;
					r1 = g_Config.m_BrSpecialWallColor >> 16;
					g1 = (g_Config.m_BrSpecialWallColor >> 8) & 0xff;
					b1 = g_Config.m_BrSpecialWallColor & 0xff;
		...
		if (g_Config.m_ClRoundingInt)
		{
			...
			Button.VSplitRight(20.0f, &Button, 0);
			str_format(aBuf, sizeof(aBuf), "%s: %i", Localize("Value"), g_Config.m_ClRounding);

			UI()->DoLabelScaled(&Label, aBuf, 13.0f, -1);
			//RightView.HSplitTop(20.0f, &Button, 0);
			Button.HMargin(2.0f, &Button);
...
void CMenus::ColorSetting(CUIRect MainView)
{
	CUIRect Button, Label;
	const float FontSize = 12.0f;
	m_SpecialPage = 1;
	...
	MainView.HSplitTop(20.0f, &Text, &MainView);
	//text.VSplitLeft(15.0f, 0, &text);
	UI()->DoLabelScaled(&Text, Localize("Color Setting:"), 14.0f, -1);
	...
	const char *paLabels[] = {
		...
		Localize("Lht."),
		Localize("Alpha")};
	float pColorSlider[4] = { m_SpecialColor.x, m_SpecialColor.y, m_SpecialColor.z, m_SpecialColor.w};
	for(int s = 0; s < 4; s++)
	{
		CUIRect Text;
		MainView.HSplitTop(19.0f, &Button, &MainView);
	...
	if(DoButton_CheckBox(0, Localize("SetColor"), 0, &Button))
	{
		m_SpecialColor = vec4(pColorSlider[1], pColorSlider[2], pColorSlider[3], pColorSlider[4]);
		m_SpecialPage = 0;
	}
}
