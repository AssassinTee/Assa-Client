...
void CServerBrowser::Update(bool ForceResort)
{
	...
	if(m_RefreshLan)
	{
		...
		for(int i = 0; i < 5; i++)
		{
			Packet.m_Address.port = m_CurrentServer;
			m_pNetClient->Send(&Packet);
			m_CurrentServer++;
			if(m_CurrentServer >= 9999 || m_StopRefresh)
			{
				m_RefreshLan = false;
				break;
			}
	...
	if(m_NeedRefresh && !m_pMasterServer->IsRefreshing() && !m_StopRefresh)
	{
		NETADDR Addr;
		CNetChunk Packet;
		int i;
	...
	while(1)
	{
		if(!pEntry || m_StopRefresh) // no more entries or forced stopping
			break;
...
CServerInfo CServerBrowser::GetInfo(const NETADDR &Addr)
{
	Request(Addr);
	return Find(Addr)->m_Info;
}
