...
class CClient : public IClient, public CDemoPlayer::IListner
{
	...
	class CNetClient m_NetClient[MAX_CLIENTS]; // dummy
...
public:
	...
	int SendMsgEx(CMsgPacker *pMsg, int Flags, bool System=true, int NetClientID = -1); // dummy
	void SendInfo();
	void SendEnterGame();
	void SendReady();
	...
	void DummyUpdateInfo();
	void DummyState();
	
	int m_DummyControlID;
	int m_DummyControlBoth;
	int m_DummyFloodBlock;
	bool m_DummyAlive;
	...
	static void Con_DummyToggleID(IConsole::IResult *pResult, void *pUserData);
	static void Con_DummyToggleBoth(IConsole::IResult *pResult, void *pUserData);
	static void Con_DummyDebug(IConsole::IResult *pResult, void *pUserData);

	static void Con_DynBind(IConsole::IResult *pResult, void *pUserData);
