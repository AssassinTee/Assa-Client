...
#include <game/generated/protocol.h> // brauchen wir für SendInfo
...
int CClient::SendMsgEx(CMsgPacker *pMsg, int Flags, bool System, int NetClientID)
{
	CNetChunk Packet;
	...
	*((unsigned char*)Packet.m_pData) <<= 1;
	...
	if(!(Flags&MSGFLAG_NOSEND))
	{
		if(NetClientID == -1)
		{
			if(m_DummyControlBoth)
			{
				for(int i = 0; i < MAX_CLIENTS; i++)
					m_NetClient[i].Send(&Packet);
			}
			else
				m_NetClient[m_DummyControlID].Send(&Packet);
		}
		else
			m_NetClient[NetClientID].Send(&Packet);
...
{
	return m_NetClient[m_DummyControlID].GotProblems() != 0;
}
...
void CClient::DummyState()
{
	m_DummyAlive = m_DummyAlive == 0 ? 1 : 0;
}
...
void CClient::DummyConnect()
{
	...
	int Port = 8303;
	NETADDR ServerAddr;
	if(m_DummyControlID || m_DummyControlBoth)
	{
		m_DummyControlID = 0;
		m_DummyControlBoth = 0;
	}
	DummyDisconnect();
	if(m_DummyAlive) {
		DummyState();
	...
	SendMsgEx(&MsgReady, MSGFLAG_VITAL|MSGFLAG_FLUSH, true, 1);
	
	// startinfo
	CNetMsg_Cl_StartInfo Msg;
	Msg.m_pName = g_Config.m_ClDummyPlayerName;
	Msg.m_pClan = g_Config.m_ClDummyPlayerClan;
	Msg.m_Country = g_Config.m_PlayerCountry; // dummy (halt die flag, die du auch hast ;o)
	Msg.m_pSkin = "vxfl!Knp";
	Msg.m_UseCustomColor = g_Config.m_ClDummyUseCustomColor;
	Msg.m_ColorBody = g_Config.m_ClDummyColorBody;
	Msg.m_ColorFeet = g_Config.m_ClDummyColorFeet;
	CMsgPacker Packer(Msg.MsgID());
	Msg.Pack(&Packer);
	SendMsgEx(&Packer, MSGFLAG_VITAL, false, 1);
	
	// send enter game an finish the connection
	CMsgPacker MsgEnter(NETMSG_ENTERGAME);
	SendMsgEx(&MsgEnter, MSGFLAG_VITAL|MSGFLAG_FLUSH, true, 1);
}

void CClient::DummyDisconnect()
{
	if(g_Config.m_ClDummyLeavemsg[0] != 0)
		m_NetClient[1].Disconnect(g_Config.m_ClDummyLeavemsg);
	else
		m_NetClient[1].Disconnect(0);
}

void CClient::DummyUpdateInfo()
{
	//
...
{
	return m_NetClient[m_DummyControlID].ErrorString();
}
...
void CClient::VersionUpdate()
{
	...
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
			Engine()->HostLookup(&m_VersionInfo.m_VersionServeraddr, g_Config.m_ClVersionServer, m_NetClient[i].NetType());
...
void CClient::InitInterfaces()
{
	...
	//
	for(int i = 0; i < 8; i++) // MAX CLIENTS = 16, thats too much
...
void CClient::Run()
{
	...
	*/
	...
	//dummy
	m_DummyControlID = 0; // standard player
	m_DummyControlBoth = 0; // just control first tee at clienstart
	m_DummyFloodBlock = 0;
...
const char *CClient::DemoPlayer_Play(const char *pFilename, int StorageType)
{
	...
	const char *pError;
	Disconnect();
	m_NetClient[m_DummyControlID].ResetErrorString();
...
{
	CClient *pSelf = (CClient *)pUserData;
	if(pSelf->m_DummyFloodBlock > pSelf->GameTick())
		return;
	dbg_msg("-.-", "k");
	pSelf->DummyConnect();
	pSelf->m_DummyFloodBlock = pSelf->GameTick() + 25;
}

void CClient::Con_DummyDisconnect(IConsole::IResult *pResult, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	pSelf->DummyDisconnect();
}

void CClient::Con_DummyToggleID(IConsole::IResult *pResult, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	if(!pSelf->m_DummyAlive)
		return;
	pSelf->m_DummyControlID = pSelf->m_DummyControlID == 0 ? 1 : 0;
}

void CClient::Con_DummyToggleBoth(IConsole::IResult *pResult, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	if(!pSelf->m_DummyAlive)
		return;
	pSelf->m_DummyControlBoth = pSelf->m_DummyControlBoth == 0 ? 1 : 0;
}

void CClient::Con_DummyDebug(IConsole::IResult *pResult, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	pSelf->m_DummyControlBoth = 0;
	pSelf->m_DummyControlID = 0;
	pSelf->m_DummyAlive = 0;
}

//Assa - this is so easy
void CClient::Con_DynBind(IConsole::IResult *pResult, void *pUserData)
{
...
void CClient::RegisterCommands()
{
	...
	//dummy
	m_pConsole->Register("dummy_connect", "", CFGFLAG_CLIENT, Con_DummyConnect, this, "connect dummy to current server");
	m_pConsole->Register("dummy_disconnect", "", CFGFLAG_CLIENT, Con_DummyDisconnect, this, "disconnect dummy from current server");
	m_pConsole->Register("dummy_toggleid", "", CFGFLAG_CLIENT, Con_DummyToggleID, this, "toggle between first and second netclient");
	m_pConsole->Register("dummy_toggleboth", "", CFGFLAG_CLIENT, Con_DummyToggleBoth, this, "toggle between control both or just one netclient");
	m_pConsole->Register("dummy_change_state_debug", "", CFGFLAG_CLIENT, Con_DummyDebug, this, "debug dummy");

	//Assa
	m_pConsole->Register("dynbind", "", CFGFLAG_CLIENT, Con_DynBind, this, "seriously does anyone read this?");
	//easteregg O 
