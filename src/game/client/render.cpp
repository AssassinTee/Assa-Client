...
{
	if (r)//bugfix
		r = min(w/2.0f, min(h/2.0f, (float)g_Config.m_ClRounding));
...
void CRenderTools::DrawUIRect(const CUIRect *r, vec4 Color, int Corners, float Rounding)
{
	...
	Graphics()->SetColor((Color.r * ((unsigned int)g_Config.m_UiColor >> 24)) / 255.0f, (Color.g * ((g_Config.m_UiColor >> 16) & 0xff)) / 255.0f, (Color.b * ((g_Config.m_UiColor >> 8) & 0xff)) / 255.0f, (g_Config.m_UiColor & 0xff) / 255.0f);
	if (!g_Config.m_ClRoundingInt) //cys
		DrawRoundRectExt(r->x, r->y, r->w, r->h, Rounding*UI()->Scale(), Corners);
	else
		DrawRoundRectExt(r->x, r->y, r->w, r->h, min(r->h / 2, min(r->w / 2, (float)g_Config.m_ClRounding))/*Rounding*/*UI()->Scale(), Corners);
	Graphics()->QuadsEnd();
}
...
void CRenderTools::RenderTilemapGenerateSkip(class CLayers *pLayers)
{
	...
	for(int g = 0; g < pLayers->NumGroups(); g++)
	{
		...
		if(!g_Config.m_BrSpecialWall)
		{
			...
			{
				CMapItemLayer *pLayer = pLayers->GetLayer(pGroup->m_StartLayer+1);
					...
					for(int y = 0; y < pTmap->m_Height; y++)
					{
						for(int x = 1; x < pTmap->m_Width;)
						{
							...
							for(sx = 1; x+sx < pTmap->m_Width && sx < 255; sx++)
							{
								/*if(g_Config.m_BrSpecialWall)
								{
									if(IsTileSolid(int x, int y))
									{
										if(pTiles[y*pTmap->m_Width+x+sx].m_Index = COLFLAG_SOLID)
										{	
											Graphics()->QuadsBegin();
										...
										else if(pTiles[y*pTmap->m_Width+x+sx].m_Index = COLFLAG_NOHOOK)
											//
										else if(pTiles[y*pTmap->m_Width+x+sx].m_Index = COLFLAG_DEATH)
											//
									}
								}*/
