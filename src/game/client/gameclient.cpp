...
//Assa Texture Addon
#include "components/entskins.h"
#include "components/bskins.h"
...
//Assa
static CAssaMessages gs_AssaMessages;
...
//Assa Texture Addon
static CentSkins gs_entSkins;
static CbSkins gs_bSkins;
...
void CGameClient::OnConsoleInit()
{
	...
	//Scoredif
	m_pScoreDif = &::gs_AssaMessages;

	// Assa Texture Addon
	m_pCentSkins = &::gs_entSkins;
	m_pbSkins = &::gs_bSkins;
...
void CGameClient::OnInit()
{
	...
	char aFilename[512];
	char aFontPath[512];
	str_format(aFontPath, sizeof(aFontPath), "fonts/%s", g_Config.m_GameFontfile);
	...
	if(!pDefaultFont)//Shit failed? Use DejaVuSans
	{
	...
	//addon
	g_pData->m_aImages[IMAGE_BACKGROUND].m_Id = m_pbSkins->Get(m_pbSkins->Find(g_Config.m_GameBack))->m_Texture;
	g_pData->m_aImages[IMAGE_ENTITIES].m_Id = m_pCentSkins->Get(m_pCentSkins->Find(g_Config.m_GameEntities))->m_Texture;
}
...
void CGameClient::OnRender()
{
	...
	// render all systems, render background first!
	if(g_Config.m_BrSpecialWall)
		{
			CUIRect Screen;
			Graphics()->GetScreen(&Screen.x, &Screen.y, &Screen.w, &Screen.h);
			//CMapItemLayerTilemap *pTMap = m_pLayers->GameLayer();
			...
			Graphics()->TextureSet(g_pData->m_aImages[IMAGE_BACKGROUND].m_Id);
			Graphics()->QuadsBegin();
			Graphics()->SetColor(1.0f, 1.0f, 1.0f, 1.0f);
			RenderTools()->DrawRoundRectExt(0, 0, sw, sh, 0.0f, 0);//Assa
			Graphics()->QuadsEnd();
	}
	for(int i = 0; i < m_All.m_Num; i++)
		...
		for(int i = 0; i < Num; i++)
		{
			...
			else if(Item.m_Type == NETOBJTYPE_PLAYERINFO)
			{
				...
				if(pInfo->m_Local)
				{
					...
					if (pInfo->m_Team == TEAM_SPECTATORS)
					{
						...
						m_Snap.m_SpecInfo.m_SpectatorID = SPEC_FREEVIEW;
						m_pScoreDif->OnSpectator();
					}
					else
                    {
                        m_pScoreDif->OnClientInfo();
                    }
				}
			...
			else if(Item.m_Type == NETOBJTYPE_GAMEINFO)
			{
				...
				if(!m_LastGameOver && m_Snap.m_pGameInfoObj->m_GameStateFlags&GAMESTATEFLAG_GAMEOVER)
					...
					OnStartGame();
				else if(m_Snap.m_pGameInfoObj->m_RoundStartTick-m_LastRoundStartTick > 2)
					OnStartGame();
...
void CGameClient::CClientData::UpdateRenderInfo(int ClientID)
{
