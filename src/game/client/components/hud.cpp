...
#include <time.h>//this is, uhm, for the weather, ofc.
...
void CHud::RenderScoreHud()
{
	...
	if(!(m_pClient->m_Snap.m_pGameInfoObj->m_GameStateFlags&GAMESTATEFLAG_GAMEOVER))
	{
		...
		if(GameFlags&GAMEFLAG_TEAMS && m_pClient->m_Snap.m_pGameDataObj)
		{
			...
			//Assa
			float FlagTimer = 0;
			if(g_Config.m_BrSpecialFlagTimer)
				FlagTimer = 25.0f;
			...
			for(int t = 0; t < 2; t++)
			{
				...
				Graphics()->QuadsBegin();
				if(CTeecompUtils::GetForceDmColors(t, m_pClient->m_Snap.m_pLocalInfo ? m_pClient->m_Snap.m_pLocalInfo->m_Team : TEAM_RED))
				{
					if(t == 0)
				...
				RenderTools()->DrawRoundRectExt(Whole-ScoreWidthMax-ImageSize-FlagTimer-2*Split, StartY+t*20, ScoreWidthMax+ImageSize+FlagTimer+2*Split, 18.0f, 5.0f, CUI::CORNER_L);//Assa
				Graphics()->QuadsEnd();
				...
				if(GameFlags&GAMEFLAG_FLAGS)
				{
					...
					// draw timer
					if(m_pClient->m_FlagDropTick[t] != 0 && g_Config.m_BrSpecialFlagTimer)
					{
						char aBuf[256];
						int Time = 29 - (Client()->GameTick()-m_pClient->m_FlagDropTick[t])/Client()->GameTickSpeed();

						if(Time < 5)
							str_format(aBuf, sizeof(aBuf), "%d.%d", Time, 9 - ((Client()->GameTick()-m_pClient->m_FlagDropTick[t])*10/Client()->GameTickSpeed())%10);//don't forget the fuckint 1 in front
						else
							str_format(aBuf, sizeof(aBuf), "%d", Time);
						if(Time >= 0)//Bugfix (??)
							TextRender()->Text(0, Whole-ScoreWidthMax-ImageSize-FlagTimer, StartY+t*20, 14.0f, aBuf, -1);
					}
...
{
	if(g_Config.m_ClWarningTeambalance)
	{
...
void CHud::RenderVoting()
{
	...
	Graphics()->TextureSet(-1);
	Graphics()->QuadsBegin();
	Graphics()->SetColor(0,0,0, m_pClient->m_pVoting->m_Alpha);
	RenderTools()->DrawRoundRect(-10, 60-2, 100+10+4+5, 46, 5.0f);
	Graphics()->QuadsEnd();

	TextRender()->TextColor(1,1,1,1);
	if(!m_pClient->m_pVoting->m_Alpha)
		TextRender()->TextColor(1, 1, 1, 0.35f);
...
void CHud::RenderHealthAndAmmo(const CNetObj_Character *pCharacter)
{
	...
	IGraphics::CQuadItem Array[400];
	int i;
	if (pCharacter->m_AmmoCount > 20){
		for (i = 0; i < min(pCharacter->m_AmmoCount, 400); i++)
			Array[i] = IGraphics::CQuadItem((float)(x + 20 * 12 * i / min(pCharacter->m_AmmoCount, 400)), y + 24, 10, 10);
	}
	else {
		for (i = 0; i < min(pCharacter->m_AmmoCount, 20); i++)
	...
	if (pCharacter->m_Health > 20){
		for (; h < min(pCharacter->m_Health, 400); h++)
			Array[h] = IGraphics::CQuadItem((float)(x + 20 * 12 * h / min(pCharacter->m_Health, 400)), y, 10, 10);
	}
	else {
		for (; h < min(pCharacter->m_Health, 20); h++)
	...
	if (pCharacter->m_Armor > 20){
		for (; h < min(pCharacter->m_Armor, 400); h++)
			Array[h] = IGraphics::CQuadItem((float)(x + 20 * 12 * h / min(pCharacter->m_Armor, 400)), y + 12, 10, 10);
	}
	else {
		for (; h < min(pCharacter->m_Armor, 20); h++)
...
void CHud::RenderTime()
{
	if(!m_pClient->m_Snap.m_pLocalInfo)
		return;//bugfix... dont understand why but ok
	if(g_Config.m_BrSpecialTimeUi)
	{
		Graphics()->TextureSet(-1);
		Graphics()->QuadsBegin();
		Graphics()->SetColor(0.0f, 0.0f, 0.0f, 0.25f);
		RenderTools()->DrawRoundRectExt(0, m_Height-10.0f, 80.0f, 10.0f, 5.0f, CUI::CORNER_TR);//Assa
		Graphics()->QuadsEnd();
		/*
		CUIRect Time, Time2;
		Time.HSplitTop(m_Height-10.0f, 0, &Time);
		Time2.HSplitTop(m_Height-10.0f, 0, &Time2);
		Time.VSplitLeft(80.0f, &Time2, &Time);
		RenderTools()->DrawUIRect(&Time2, vec4(0.0f, 0.0f, 0.0f, 0.5f), CUI::CORNER_TR, 10.0f);*/
	}
	...
	str_format(aComp, sizeof(aComp), "%sUhr - %s", aTime, aDate);
