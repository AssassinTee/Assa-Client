...
void CPlayers::RenderFeet(CTeeRenderInfo *pInfo, vec2 Dir, vec2 Pos, float Angle)
{
	// draw feet
	vec2 Direction = Dir;
	vec2 Position = Pos;
	...
	//if(OutLine)
	RenderTools()->SelectSprite(SPRITE_TEE_FOOT_OUTLINE, 0, 0, 0);
	/*
	else
	{
		RenderTools()->SelectSprite(SPRITE_TEE_FOOT, 0, 0, 0);
		if(Indicate)
			cs = 0.5f;
	} 
	*/
	Graphics()->SetColor(pInfo->m_ColorFeet.r*cs, pInfo->m_ColorFeet.g*cs, pInfo->m_ColorFeet.b*cs, pInfo->m_ColorFeet.a);
	IGraphics::CQuadItem QuadItem(Position.x, Position.y, w, h);
	Graphics()->QuadsDraw(&QuadItem, 1);
}
