...
void CMenus::RenderSettingsTeecomp(CUIRect MainView)
{
	...
	const char *pTabs[] = { Localize("Skins"), Localize("Stats"), Localize("Scores"), Localize("About") };
...
void CMenus::RenderSettingsTeecompAbout(CUIRect MainView)
{
	...
	MainView.HSplitTop(20.0f, &Button, &MainView);
	UI()->DoLabel(&Button, Localize("(small changings by Assa, because Special is older then TeeComp)"), 14.0f, 0);
