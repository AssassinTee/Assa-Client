...
void CEffects::PlayerDeath(vec2 Pos, int ClientID)
{
	...
	if(ClientID >= 0)
	{
		if(g_Config.m_BrSpecialBlood)
		{
			...
			{
				BloodColor = vec3(1.0f,0,0);
				SplashNumber = 256;
			}
			else
			{
				BloodColor.r = (g_Config.m_BrSpecialColorBlood>>16)/255.0f;
				BloodColor.g = ((g_Config.m_BrSpecialColorBlood>>8)&0xff)/255.0f;
				BloodColor.b = (g_Config.m_BrSpecialColorBlood&0xff)/255.0f;
				SplashNumber = 256;
			}
		}
		else if(m_pClient->m_aClients[ClientID].m_UseCustomColor)
	...
	for(int i = 0; i < SplashNumber; i++)
	{
		...
		if(g_Config.m_BrSpecialBlood)
		{
			p.m_Vel = RandomDir() * ((frandom()+0.1f)*1800.0f);
			p.m_LifeSpan = 0.7f + frandom()*0.7f;
		}
	...
	//CPlayers b;
	//b->RenderFeet(m_pClient->m_aClients[ClientID].m_ClientInfo, vec2 Dir, vec2 Pos, float Angle);
