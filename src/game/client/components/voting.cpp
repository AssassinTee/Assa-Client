...
{
	m_Alpha = 0.75f;//Werden noch mit dem Menu Alpha verrechnet
	if(m_PressedButton)
		m_Alpha = 0;
		
	RenderTools()->DrawUIRect(&Bars, vec4(0,0,0, m_Alpha-0.25f), CUI::CORNER_ALL, Bars.h/3);

	CUIRect Splitter = Bars;
	Splitter.x = Splitter.x+Splitter.w/2;
	Splitter.w = Splitter.h/2.0f;
	Splitter.x -= Splitter.w/2;
	RenderTools()->DrawUIRect(&Splitter, vec4(0,0,0, m_Alpha-0.25f), CUI::CORNER_ALL, Splitter.h/4);
	...
	if(m_Total)
	{
		...
		if(m_Yes)
		{
			...
			RenderTools()->DrawUIRect(&YesArea, vec4(1.0f,1.0f,1.0f, min(m_Alpha + 0.4f, 1.0f)), CUI::CORNER_ALL, Bars.h/3);
		...
		if(m_No)
		{
			...
			RenderTools()->DrawUIRect(&NoArea, vec4(1.0f,1.0f,1.0f, min(m_Alpha + 0.4f, 1.0f)), CUI::CORNER_ALL, Bars.h/3);
