...
void CKillMessages::OnReset()
{
	...
	for(int i = 0; i < MAX_KILLMSGS; i++)
	{
		...
		m_aKillmsgs[i].m_AssaTick = -100000;
		m_OldScore = 0;
		m_ScoreDif[i] = 0;
	}
}
...
void CKillMessages::OnRender()
{
    ...
    int r1, g1, b1;
    r1 = g_Config.m_BrSpecialKillMarkColor >> 16;
    g1 = (g_Config.m_BrSpecialKillMarkColor >> 8) & 0xff;
    b1 = g_Config.m_BrSpecialKillMarkColor & 0xff;

    int r2, g2, b2;
    r2 = g_Config.m_BrSpecialKillMarkDColor >> 16;
    g2 = (g_Config.m_BrSpecialKillMarkDColor >> 8) & 0xff;
    b2 = g_Config.m_BrSpecialKillMarkDColor & 0xff;
	...
	if(g_Config.m_BrSpecialKillMessage)
	{
		for(int i = 1; i <= MAX_KILLMSGS; i++)
		{
			...
			// render victim name
			x -= VictimNameW;
			if(g_Config.m_BrSpecialKillMarkD && m_pClient->m_Snap.m_pLocalInfo && m_pClient->m_Snap.m_pLocalInfo->m_ClientID == m_aKillmsgs[r].m_VictimID)
			...
			if(m_aKillmsgs[r].m_VictimID != m_aKillmsgs[r].m_KillerID)
			{
				...
				// render killer name
				x -= KillerNameW;
				if(g_Config.m_BrSpecialKillMark && m_pClient->m_Snap.m_pLocalInfo && m_pClient->m_Snap.m_pLocalInfo->m_ClientID == m_aKillmsgs[r].m_KillerID)
