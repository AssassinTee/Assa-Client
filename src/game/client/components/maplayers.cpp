...
void CMapLayers::OnRender()
{
	...
	for(int g = 0; g < m_pLayers->NumGroups(); g++)
	{
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if (Render && !IsGameLayer && (!g_Config.m_BrSpecialWall || g_Config.m_BrSpecialWallTiles || g_Config.m_BrSpecialWallQuads))
			{
				//layershot_begin();

				if (pLayer->m_Type == LAYERTYPE_TILES && (!g_Config.m_BrSpecialWall || (g_Config.m_BrSpecialWall && g_Config.m_BrSpecialWallTiles)))
				{
					CMapItemLayerTilemap *pTMap = (CMapItemLayerTilemap *)pLayer;
					if(pTMap->m_Image == -1)
					...
					Graphics()->BlendNone();
					vec4 Color;
					if (g_Config.m_BrSpecialWallTiles && g_Config.m_BrSpecialWall)
						Color = vec4(pTMap->m_Color.r / 255.0f, pTMap->m_Color.g / 255.0f, pTMap->m_Color.b / 255.0f, pTMap->m_Color.a / 255.0f / 2.0f);
					else
				...
				else if (pLayer->m_Type == LAYERTYPE_QUADS && (!g_Config.m_BrSpecialWall || g_Config.m_BrSpecialWallQuads))
				{
					...
					Graphics()->BlendNone();
					int NumQuads = pQLayer->m_NumQuads;

					if (pQLayer->m_Image == -1 && g_Config.m_BrSpecialWallColorQuads)
					{
						int r, g, b;
						r = g_Config.m_BrSpecialWallColor >> 16;
						g = (g_Config.m_BrSpecialWallColor >> 8) & 0xff;
						b = g_Config.m_BrSpecialWallColor & 0xff;
					...
					RenderTools()->RenderQuads(pQuads, NumQuads, LAYERRENDERFLAG_OPAQUE, EnvelopeEval, this);
					Graphics()->BlendNormal();
					RenderTools()->RenderQuads(pQuads, NumQuads, LAYERRENDERFLAG_TRANSPARENT, EnvelopeEval, this);
				}

				//layershot_end();
			}
			else if(IsGameLayer && g_Config.m_BrSpecialWall && g_Config.m_BrSpecialWallSolid)
			{
				CMapItemLayerTilemap *pTMap = (CMapItemLayerTilemap *)pLayer;
				Graphics()->TextureSet(g_pData->m_aImages[IMAGE_ENTITIES].m_Id);
				CTile *pTiles = (CTile *)m_pLayers->Map()->GetData(pTMap->m_Data);

				Graphics()->BlendNone();
				vec4 Color = vec4(1, 1, 1, 0.75f);
				RenderTools()->RenderTilemap(pTiles, pTMap->m_Width, pTMap->m_Height, 32.0f, Color, TILERENDERFLAG_EXTEND|LAYERRENDERFLAG_OPAQUE,
												EnvelopeEval, this, pTMap->m_ColorEnv, pTMap->m_ColorEnvOffset);
				Graphics()->BlendNormal();
				RenderTools()->RenderTilemap(pTiles, pTMap->m_Width, pTMap->m_Height, 32.0f, Color, TILERENDERFLAG_EXTEND|LAYERRENDERFLAG_TRANSPARENT,
