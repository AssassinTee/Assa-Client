...
void CMenus::RenderRgbaSliders(CUIRect* pMainView, CUIRect* pButton, int &r, int &g, int &b, int &a, bool Enabled)
{
	const char *pLabels[] = { "R.", "G.", "B.", "A." };
	int *pColorSlider[4] = { &r, &g, &b , &a};
	for (int i = 0; i<4; i++)
	{
		CUIRect Text;
		pMainView->HSplitTop(19.0f, pButton, pMainView);
...
int CMenus::RenderMenubar(CUIRect r)
{
	...
	if(Client()->State() == IClient::STATE_OFFLINE)
	{
		...
		if(DoButton_MenuTab(&s_RecentButton, Localize("Recent"), m_ActivePage==PAGE_RECENT, &Button, CUI::CORNER_TR))
		{
	...
	else
	{
		...
		if(DoButton_MenuTab(&s_ServersButton, Localize("Servers"), m_ActivePage==PAGE_INTERNET, &Button, 0))
			NewPage = PAGE_INTERNET;
...
void CMenus::RenderLoading()
{
	...
	// need up date this here to get correct //Assa
	...
	ms_GuiColor = vec4((unsigned int)(g_Config.m_UiColor >> 24) / 255.0f, (((g_Config.m_UiColor >> 16) & 0xff)) / 255.0f, (((g_Config.m_UiColor >> 8) & 0xff)) / 255.0f, (g_Config.m_UiColor & 0xff) / 255.0f);
	...
	static int sTextureLoading = Graphics()->LoadTexture("loading_bg.png", IStorage::TYPE_ALL, CImageInfo::FORMAT_AUTO, 0);
	Graphics()->TextureSet(sTextureLoading);
	Graphics()->QuadsBegin();
	//Graphics()->SetColor(1,1,1,1);
	RenderTools()->DrawRoundRect(Screen.x, Screen.y, Screen.w, Screen.h, 0);
	Graphics()->QuadsEnd();
	/*
	char aBuf[128];
	str_format(aBuf, sizeof(aBuf), "Wide: %s, Hight: %s", Screen.w, Screen.h);
	Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "game", aBuf);
	*/
	char aBuf[128];
	str_format(aBuf, sizeof(aBuf), "Assa Client: Version %s", Localize(ASSA_VERSION));
	const char *pCaption = Localize(aBuf);
	...
	Graphics()->QuadsBegin();
	Graphics()->SetColor(0,1,0,0.75f);
	RenderTools()->DrawRoundRect(Screen.x+40, Screen.y+Screen.h-75, (Screen.w-80)*Percent, 25, 5.0f);
	Graphics()->QuadsEnd();
...
int CMenus::Render()
{
	...
	else
	{
		...
		else if(m_Popup == POPUP_CONNECTING)
		{
			...
			{
				if(i = 0)
					RecentAdress[i][64] = g_Config.m_UiServerAddress;
				else
					RecentAdress[i][64] = RecentAdress[i-1][64];
			}*/
			
			static bool gotServerInfo = false;
			if (!gotServerInfo && str_find_nocase(g_Config.m_UiServerAddress, "localhost") == 0 && str_length(g_Config.m_UiServerAddress) > 5)//you can still connect with ":PORT"
			{
				NETADDR Addr;
				net_addr_from_str(&Addr, g_Config.m_UiServerAddress);
				CServerInfo server = ServerBrowser()->GetInfo(Addr);
				gotServerInfo = true;
			}
		...
		{
			static int64 now1 = 0;
			if (g_Config.m_ClAutoReconnect && now1 + time_freq()/25.0f < time_get() )//every second tick should be enough
			{
				NETADDR Addr;
				net_addr_from_str(&Addr, g_Config.m_UiServerAddress);
				CServerInfo server = ServerBrowser()->GetInfo(Addr);
				if (server.m_NumClients < server.m_MaxClients)
				{
			...
			static int64 now = 0;
			if (now + time_freq()/2.0f < time_get()) 
			{
				b++;
				mem_zero(buffer, sizeof(buffer));
				for (int i = 0; i < b % 4; i++)
				{
					str_format(buffer, sizeof(buffer), "%s%s", buffer, ".");
				}
				now = time_get();
			}
			pTitle = g_Config.m_ClAutoReconnect ? Localize("Autoreconnect") : Localize("Disconnected");
			char aBuf[64];
			str_format(aBuf, sizeof(aBuf), "%s - %s %s %s", Localize(Client()->ErrorString()), Localize("Connecting to"), g_Config.m_UiServerAddress, buffer);
			pExtraText = aBuf;
			pButtonText = Localize("Abort");
			ExtraAlign = -1;
		}
