...
void CMenus::RenderSettingsControls(CUIRect MainView)
{
	...
	// movement settings
	{
...

		{
			...
			Button.HMargin(2.0f, &Button);
			g_Config.m_InpMousesens = (int)(DoScrollbarH(&g_Config.m_InpMousesens, &Button, (g_Config.m_InpMousesens-5)/1000.0f)*1000.0f)+5;
...
void CMenus::RenderSettingsGraphics(CUIRect MainView)
{
	...
	//handled like in teecomp...
	MainView.HSplitTop(20.0f, &Text, &MainView);
	MainView.HSplitTop(19.0f, &Button, &MainView);
	static int r, g, b, a;
	r = (unsigned int)g_Config.m_UiColor >> 24;
	g = (g_Config.m_UiColor >> 16) & 0xff;
	b = (g_Config.m_UiColor >> 8 ) & 0xff;
	a = g_Config.m_UiColor & 0xff;
	RenderRgbaSliders(&MainView, &Button, r, g, b, a, true);//true or false? i don't know
	g_Config.m_UiColor = (r << 24) + (g << 16) + (b << 8) + a;
}
...
void CMenus::RenderSettingsSpecial(CUIRect MainView)
{
	CUIRect Button, Label, Left, Midleft, Right, Players, Killmessage, Gametiles, Other;//Players//Gametiles//Other
	float y=95.0f;
	MainView.HSplitTop(y, &Players, &Killmessage);
	{//Players

		// headline
		Players.HSplitTop(25.0f, &Label, &Players);
		UI()->DoLabelScaled(&Label, Localize("Player Settings"), 15.0f, -1);
		Players.Margin(5.0f, &Players);
		Players.VSplitMid(&Left, &Right);
		Left.VSplitRight(5.0f, &Left, 0);
		Right.VMargin(5.0f, &Right);

		//Player score
		Left.HSplitTop(20.0f, &Button, &Left);
		if (DoButton_CheckBox(&g_Config.m_BrSpecialScore, Localize("Show Score in Nameplates"), g_Config.m_BrSpecialScore, &Button))
			g_Config.m_BrSpecialScore ^= 1;

		//Player blood
		Left.HSplitTop(20.0f, &Button, &Left);
		if (DoButton_CheckBox(&g_Config.m_BrSpecialBlood, Localize("Use Own blood"), g_Config.m_BrSpecialBlood, &Button))
			g_Config.m_BrSpecialBlood ^= 1;

		//Nameplate Color
		Left.HSplitTop(20.0f, &Button, &Left);
		if (g_Config.m_BrSpecialNameplateColor)
		{
			vec4 A = m_pClient->m_pSkins->GetColorV4(g_Config.m_PlayerColorBody);//well, don't make it too dificult :)
			TextRender()->TextColor(A.x, A.y, A.z, A.a); //TODO: Ownskin color
		}
		if (DoButton_CheckBox(&g_Config.m_BrSpecialNameplateColor, Localize("Color non-team nameplates"), g_Config.m_BrSpecialNameplateColor, &Button))
			g_Config.m_BrSpecialNameplateColor ^= 1;
		TextRender()->TextColor(1, 1, 1, 1);

		//Right
		//Blood Color
		if (g_Config.m_BrSpecialBlood)
		{
			Right.HSplitTop(20.0f, &Button, &Right);
			if(g_Config.m_BrSpecialRedBlood)
            {
            ...
            {
                int r1, g1, b1;
				CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialColorBlood);
				TextRender()->TextColor(r1 / 255.0f, g1 / 255.0f, b1 / 255.0f, 1.0f);
            }
			if (DoButton_CheckBox(&g_Config.m_BrSpecialRedBlood, Localize("Use Red blood"), g_Config.m_BrSpecialRedBlood, &Button))
			{
				g_Config.m_BrSpecialRedBlood ^= 1;
			}
			if (!g_Config.m_BrSpecialRedBlood)
			{
				//handled like in teecomp...
				static int r, g, b;
				CMenus::SetColorVar(&r, &g, &b, &g_Config.m_BrSpecialColorBlood);
				RenderRgbSliders(&Right, &Button, r, g, b, true);
				g_Config.m_BrSpecialColorBlood = (r << 16) + (g << 8) + b;
			}
		}
	...
	{//Killmessage
		// headline
		Killmessage.HSplitTop(25.0f, &Label, &Killmessage);
		UI()->DoLabelScaled(&Label, Localize("Killmessage-Options:"), 15.0f, -1);
		Killmessage.Margin(5.0f, &Killmessage);
		Killmessage.VSplitMid(&Left, &Right);
		Left.VSplitRight(5.0f, &Left, 0);
		Right.VMargin(5.0f, &Right);

		//ShowKillmsges
		Left.HSplitTop(20.0f, &Button, &Left);
		if (DoButton_CheckBox(&g_Config.m_BrSpecialKillMessage, Localize("Show Killmessages"), g_Config.m_BrSpecialKillMessage, &Button))
			g_Config.m_BrSpecialKillMessage ^= 1;

		if (g_Config.m_BrSpecialKillMessage)
		{
			Left.VSplitLeft(20.0f, &Button, &Left);
			if (g_Config.m_BrSpecialKillMark)
            {
				int r1, g1, b1;
				CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialKillMarkColor);
				TextRender()->TextColor(r1 / 255.0f, g1 / 255.0f, b1 / 255.0f, 1.0f);
            }
			Left.HSplitTop(20.0f, &Button, &Left);
			if (DoButton_CheckBox(&g_Config.m_BrSpecialKillMark, Localize("Mark own kills"), g_Config.m_BrSpecialKillMark, &Button))
				g_Config.m_BrSpecialKillMark ^= 1;

            if(g_Config.m_BrSpecialKillMark)
            {
				static int r1, g1, b1;
				CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialKillMarkColor);
				RenderRgbSliders(&Left, &Button, r1, g1, b1, true);//true or false? i don't know
				g_Config.m_BrSpecialKillMarkColor = (r1 << 16) + (g1 << 8) + b1;
            }

			if (g_Config.m_BrSpecialKillMarkD)
            {
                int r1, g1, b1;
				CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialKillMarkDColor);
				TextRender()->TextColor(r1 / 255.0f, g1 / 255.0f, b1 / 255.0f, 1.0f);
            }
			Right.HSplitTop(20.0f, &Button, &Right);
			if (DoButton_CheckBox(&g_Config.m_BrSpecialKillMarkD, Localize("Mark own deaths"), g_Config.m_BrSpecialKillMarkD, &Button))
				g_Config.m_BrSpecialKillMarkD ^= 1;

            if(g_Config.m_BrSpecialKillMarkD)
            {
				static int r1, g1, b1;
				CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialKillMarkDColor);
				RenderRgbSliders(&Right, &Button, r1, g1, b1, true);//true or false? i don't know
				g_Config.m_BrSpecialKillMarkDColor = (r1 << 16) + (g1 << 8) + b1;
            }
		}
		TextRender()->TextColor(1, 1, 1, 1);
	...
	y+=130.0f;
	MainView.HSplitTop(y, &Players, &Gametiles);

	{//Gametiles
		//headline
		Gametiles.HSplitTop(25.0f, &Label, &Gametiles);
		UI()->DoLabelScaled(&Label, Localize("Show Gametiles:"), 15.0f, -1);
		Gametiles.Margin(5.0f, &Gametiles);
		Gametiles.VSplitMid(&Left, &Right);
		Left.VSplitRight(5.0f, &Left, 0);
		Right.VMargin(5.0f, &Right);

		//Only Show gametiles
		Left.HSplitTop(20.0f, &Button, &Left);
		if (DoButton_CheckBox(&g_Config.m_BrSpecialWall, Localize("Only show gametiles"), g_Config.m_BrSpecialWall, &Button))
			g_Config.m_BrSpecialWall ^= 1;

		if (g_Config.m_BrSpecialWall)
		{
			Left.VSplitLeft(20.0f, &Button, &Left);
			Left.HSplitTop(20.0f, &Button, &Left);
			//tileset
			if (DoButton_CheckBox(&g_Config.m_BrSpecialWallTiles, Localize("Show tileset"), g_Config.m_BrSpecialWallTiles, &Button))
				g_Config.m_BrSpecialWallTiles ^= 1;

			Left.HSplitTop(20.0f, &Button, &Left);
			//entities
			if (DoButton_CheckBox(&g_Config.m_BrSpecialWallSolid, Localize("Show Entities"), g_Config.m_BrSpecialWallSolid, &Button))
				g_Config.m_BrSpecialWallSolid ^= 1;

			Left.HSplitTop(20.0f, &Button, &Left);
			if (DoButton_CheckBox(&g_Config.m_BrSpecialWallQuads, Localize("Show Quads"), g_Config.m_BrSpecialWallQuads, &Button))
				g_Config.m_BrSpecialWallQuads ^= 1;
		}


		if (g_Config.m_BrSpecialWallQuads && g_Config.m_BrSpecialWall)
		{

			if (g_Config.m_BrSpecialWallColorQuads)
			{
				int r1, g1, b1;
				CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialWallColor);
				TextRender()->TextColor(r1 / 255.0f, g1 / 255.0f, b1 / 255.0f, 1.0f);
			}
			Right.HSplitTop(20.0f, &Button, &Right);
			if (DoButton_CheckBox(&g_Config.m_BrSpecialWallColorQuads, Localize("Color Quads"), g_Config.m_BrSpecialWallColorQuads, &Button))
				g_Config.m_BrSpecialWallColorQuads ^= 1;
			if (g_Config.m_BrSpecialWallColorQuads)
			{
				//MainView.HSplitTop(20.0f, &Button, &MainView);
				//handled like in teecomp...
				...
				CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialWallColor);
				RenderRgbSliders(&Right, &Button, r1, g1, b1, true);//true or false? i don't know
				g_Config.m_BrSpecialWallColor = (r1 << 16) + (g1 << 8) + b1;
			}
		}
		TextRender()->TextColor(1, 1, 1, 1.0f);
	}
    y+=120.0f;
	MainView.HSplitTop(y, &Players, &Other);

	{//Other
		//headline
		Other.HSplitTop(25.0f, &Label, &Other);
		UI()->DoLabelScaled(&Label, Localize("Others:"), 15.0f, -1);
		Other.Margin(5.0f, &Other);
		Other.VSplitMid(&Left, &Right);
		Left.VSplitRight(5.0f, &Left, 0);
		Right.VMargin(5.0f, &Right);

		//Flag return time
		Left.HSplitTop(20.0f, &Button, &Left);
		if (DoButton_CheckBox(&g_Config.m_BrSpecialFlagTimer, Localize("Show flag-return time"), g_Config.m_BrSpecialFlagTimer, &Button))
			g_Config.m_BrSpecialFlagTimer ^= 1;
		...
		Left.HSplitTop(20.0f, &Button, &Left);
		if (DoButton_CheckBox(&g_Config.m_ClWarningTeambalance, Localize("Show Team-Balance Warning"), g_Config.m_ClWarningTeambalance, &Button))
			g_Config.m_ClWarningTeambalance ^= 1;

		//Rounded Corners
		Left.HSplitTop(20.0f, &Button, &Left);
		if (DoButton_CheckBox(&g_Config.m_ClRoundingInt, Localize("Enable corner-size:"), g_Config.m_ClRoundingInt != 0, &Button))
			g_Config.m_ClRoundingInt ^= 1;


		char aBuf[64];
		if (g_Config.m_ClRoundingInt)
		{
			str_format(aBuf, sizeof(aBuf), "%s: %d", Localize("Value"), g_Config.m_ClRounding);
			UI()->DoLabelScaled(&Left, aBuf, 12.0f, -1);
			Left.VSplitLeft(60.0f, &Button, &Left);
			Left.HSplitTop(20.0f, &Button, &Left);
			g_Config.m_ClRounding = static_cast<int>(DoScrollbarH(&g_Config.m_ClRounding, &Button, g_Config.m_ClRounding / 20.0f)*20.0f + 0.1f);
			Left.VSplitLeft(-60.0f, &Button, &Left);
		}

		//Clock
		Left.HSplitTop(20.0f, &Button, &Left);
		if (DoButton_CheckBox(&g_Config.m_BrSpecialTime, Localize("Show ingame clock"), g_Config.m_BrSpecialTime, &Button))
			g_Config.m_BrSpecialTime ^= 1;

		//Clock UI
		if (g_Config.m_BrSpecialTime)
		{
			Left.VSplitLeft(20.0f, &Button, &Left);
			Left.HSplitTop(20.0f, &Button, &Left);
			//Left.VSplitLeft(20.0f, &Button, &Left);
			if (DoButton_CheckBox(&g_Config.m_BrSpecialTimeUi, Localize("Show ingame clock-Ui"), g_Config.m_BrSpecialTimeUi, &Button))
				g_Config.m_BrSpecialTimeUi ^= 1;
		}

		//Dynbind
		UI()->DoLabel(&Right, Localize("Dynamic camera bind-key:"), 12.0f, -1);
		//Right.VSplitLeft(120.0f, 0, &Right);
		CKeyInfo pKeys[] = { { "", "dynbind", 0 } };
		Right.HSplitTop(20.0f, &Button, &Right);
		...
		for (unsigned int i = 0; i<sizeof(pKeys) / sizeof(CKeyInfo); i++)
			UiDoKeybinder(pKeys[i], &Button);

        //Show Score addition
		Right.HSplitTop(20.0f, &Button, &Right);
		if (g_Config.m_BrSpecialKillAssa)
        {
			int r1, g1, b1;
            CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialKillAssaColor);
            TextRender()->TextColor(r1 / 255.0f, g1 / 255.0f, b1 / 255.0f, 1.0f);
        }
		if (DoButton_CheckBox(&g_Config.m_BrSpecialKillAssa, Localize("Show Scoring"), g_Config.m_BrSpecialKillAssa, &Button))
			g_Config.m_BrSpecialKillAssa ^= 1;
        ...
        if (g_Config.m_BrSpecialKillAssa)
        {
            ...
            CMenus::SetColorVar(&r1, &g1, &b1, &g_Config.m_BrSpecialKillAssaColor);
            RenderRgbSliders(&Right, &Button, r1, g1, b1, true);//true or false? i don't know
            g_Config.m_BrSpecialKillAssaColor = (r1 << 16) + (g1 << 8) + b1;
        }
        TextRender()->TextColor(1, 1, 1, 1);
	}
}
...
{
	CUIRect Button, ResetButton, Label, Right, Left, Down, Top;
	MainView.HSplitTop(10.0f, 0, &MainView);
	bool NeedSendinfo;
	{
		// player name
		...
		UI()->DoLabelScaled(&Label, aBuf, 14.0, -1);
		static float s_OffsetName = 0.0f;
		if(DoEditBox(g_Config.m_ClDummyPlayerName, &Button, g_Config.m_ClDummyPlayerName, sizeof(g_Config.m_ClDummyPlayerName), 14.0f, &s_OffsetName))
			NeedSendinfo = true;
	}
	{
		MainView.HSplitTop(-20.0f, 0, &MainView);
		MainView.HSplitTop(20.0f, &Button, &MainView);
		Button.VSplitLeft(320.0f, &Label, &Button);
		Label.VSplitLeft(235.0f, 0, &Label);
		Button.VSplitLeft(150.0f, &Button, 0);
		char aBuf[128];
		str_format(aBuf, sizeof(aBuf), "%s:", Localize("Clan"));
		UI()->DoLabelScaled(&Label, aBuf, 14.0, -1);
		static float s_OffsetName = 0.0f;
		if(DoEditBox(g_Config.m_ClDummyPlayerClan, &Button, g_Config.m_ClDummyPlayerClan, sizeof(g_Config.m_ClDummyPlayerClan), 14.0f, &s_OffsetName))
			NeedSendinfo = true;
	}

	MainView.HSplitTop(-20.0f, 0, &MainView);
	MainView.HSplitTop(20.0f, &Button, &MainView);
	Button.VSplitLeft(480.0f, &Label, &Button);
	Button.VSplitLeft(150.0f, &Button, 0);
	static int s_SetButton= 0;
	if(DoButton_Menu(&s_SetButton, Localize("Update"), 0, &Button))
		Client()->DummyUpdateInfo();

	// skin info
	const CSkins::CSkin *pOwnSkin = m_pClient->m_pSkins->Get(m_pClient->m_pSkins->Find(g_Config.m_ClDummyPlayerSkin));
	CTeeRenderInfo OwnSkinInfo;
	...
	Label.VSplitLeft(230.0f, &Label, 0);
	char aBuf[128];
	str_format(aBuf, sizeof(aBuf), "%s:", Localize("Playerskin"));
	UI()->DoLabelScaled(&Label, aBuf, 14.0f, -1);
	...
	Label.HSplitTop(15.0f, 0, &Label);;
	Label.VSplitLeft(70.0f, 0, &Label);
	UI()->DoLabelScaled(&Label,  g_Config.m_ClDummyPlayerSkin, 14.0f, -1, 150.0f);

	// custom colour selector
	MainView.HSplitTop(20.0f, 0, &MainView);
	MainView.HSplitTop(20.0f, &Button, &MainView);
	Button.VSplitLeft(230.0f, &Button, 0);
	if(DoButton_CheckBox(&g_Config.m_ClDummyColorBody, Localize("Custom colors"), g_Config.m_ClDummyUseCustomColor, &Button))
	{
		g_Config.m_ClDummyUseCustomColor = g_Config.m_ClDummyUseCustomColor?0:1;
		NeedSendinfo = true;
	}
	...
	if(g_Config.m_ClDummyUseCustomColor)
	{
		...
		int *paColors[2];
		paColors[0] = &g_Config.m_ClDummyColorBody;
		paColors[1] = &g_Config.m_ClDummyColorFeet;
	...
	//MainView.HSplitTop(20.0f, &Button, &MainView);
	//MainView.VSplitLeft(320, 0, &MainView);
	//Button.VSplitLeft(490.0f, &Label, &Button);
	//Button.VSplitLeft(150.0f, &Button, 0);

	vec3 Color = m_pClient->m_pSkins->GetColorV3(g_Config.m_ClDummyColorBody);
	TextRender()->TextColor(Color.r, Color.g, Color.b, 1.0f);
	UI()->DoLabel(&MainView, Localize("Dummy Binds:"), 16.0f, -1);
	//UI()->DoLabel(&MainView, Localize("______________"), 16.0f, -1);
	TextRender()->TextColor(1, 1, 1, 1.0f);

	MainView.VSplitLeft(150, 0, &MainView);
	char aaBuf[3][32];
	str_format(aaBuf[0], sizeof(aaBuf[0]), "%s:", Localize("Spawn/Delete Dummy"));
	str_format(aaBuf[1], sizeof(aaBuf[1]), "%s:", Localize("Swap with D."));
	str_format(aaBuf[2], sizeof(aaBuf[2]), "%s:", Localize("Play both"));
	CKeyInfo pKeys[] = {{ aaBuf[0], "dummy_connect", 0},
		{ aaBuf[1], "dummy_toggleid", 0},
		{ aaBuf[2], "dummy_toggleboth", 0}};
	...
	MainView.VSplitLeft(-150, 0, &MainView);
	//MainView.HSplitTop(-100, 0, &MainView);
	//MainView.HSplitTop(70.0f, 0, &MainView);
	...
	for(int i = 0; i < s_paSkinList.size(); ++i)
	{
		...
		if(str_comp(s->m_aName, g_Config.m_ClDummyPlayerSkin) == 0)
			OldSelected = i;
		...
		if(Item.m_Visible)
		{
			...
			{
				Info.m_Texture = s->m_ColorTexture;
				Info.m_ColorBody = m_pClient->m_pSkins->GetColorV4(g_Config.m_ClDummyColorBody);
				Info.m_ColorFeet = m_pClient->m_pSkins->GetColorV4(g_Config.m_ClDummyColorFeet);
			}
			else
			{
				Info.m_Texture = s->m_OrgTexture;
			...
			if(g_Config.m_Debug)
			{
				vec3 BloodColor = g_Config.m_ClDummyUseCustomColor ? m_pClient->m_pSkins->GetColorV3(g_Config.m_ClDummyColorBody) : s->m_BloodColor;
				Graphics()->TextureSet(-1);
				Graphics()->QuadsBegin();
				Graphics()->SetColor(BloodColor.r, BloodColor.g, BloodColor.b, 1.0f);
				IGraphics::CQuadItem QuadItem(Item.m_Rect.x, Item.m_Rect.y, 12.0f, 12.0f);
	...
	if(OldSelected != NewSelected)
	{
		mem_copy(g_Config.m_ClDummyPlayerSkin, s_paSkinList[NewSelected]->m_aName, sizeof(g_Config.m_ClDummyPlayerSkin));
		NeedSendinfo = true;
	}
}
...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	if(s_SettingsPage == 0)
		...
		RenderSettingsSpecial(MainView);
	else if(s_SettingsPage == 9)
		RenderSettingsTeecomp(MainView);
	else if(s_SettingsPage == 10)
		RenderSettingsDummyPlayer(MainView);//Yes i love tabs
...
void CMenus::SetColorVar(int *r, int *g, int *b, int *var)
{
    *r = *var >> 16;
    *g = (*var >> 8) & 0xff;
    *b = *var & 0xff;
